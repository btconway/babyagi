...
## Major update: Streamlined Google search functionality with reasoning capabilities added
...
In the meantime I did modify almost all agent prompts, some extensively, with the goal to optimize the task management for the finite process. In addition to what is implemented in the main branch, I tried to instruct the AI to:
...
CONTRIBUTION_THRESHOLD=The threshold (percentage) for last task result's contribution to the ultimate objective, for changing the topic for new tasks in case of low contribution (0 means only in case of no contribution and deactivates the feature)
